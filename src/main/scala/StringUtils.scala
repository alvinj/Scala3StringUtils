...
object StringUtils:
     ...
     *
     * @param s A non-null input string.
     * @param lengthRequested The maximum number of characters to return, 
     *        expected to be `1` or higher. `0` or less returns a blank string.
     */
    def truncateWithEllipsis(s: String, lengthRequested: Int): String =
        if s.length == 0 || lengthRequested == 0 then
            ""
        else if lengthRequested < s.length then
    ...
    // TODO if you want to pursue handling those characters, you’ll need to 
    // use pattern-matching, something like this (capture the whitespace characters 
    // in the input string and then replace them in the output string).
    ...
    /**
      ...
      */
    def convertCamelCaseToUnderscore(s: String): String = 
...
//    }
     ...
     */
    def booleanAsYOrN(b: Boolean): String = if b then "y" else "n"
    ...
    def isYes(userInput: String, default: Boolean): Boolean =
        if userInput.trim.equalsIgnoreCase("Y") || userInput.trim.equalsIgnoreCase("yes") then
            ...
            // the user can type in "ye" or "foo" or anything else
            default
        end if
