...
class StringUtilsTests extends AnyFunSuite {
    ...
    test("`truncate`: strLen < desiredLen") {
        ...
        result = truncate("abc", Int.MaxValue);  assert(result == "abc")
    }
    ...
    test("`truncate`: strLen > desiredLen") {
        ...
        result = truncate(chars,  3);  assert(result == "abc")
    }
    ...
    test("`truncate` w/ desiredLen=0 should return an empty string") {
        ...
        result = truncate(chars, 0); assert(result == "")
    }

    test("`truncate` w/ desiredLen<0 should return an empty string") {
        var result = ""
        result = truncate("", -1);    assert(result == "")
        result = truncate("a", -2);   assert(result == "")
        result = truncate(chars, Int.MinValue); assert(result == "")
    }
    ...
    test("`truncateWithEllipsis`: test normal expected behavior") {
        var result = ""
        result = truncateWithEllipsis("aleka", 0);   assert(result == "")
        ...
        result = truncateWithEllipsis("aleka", Int.MaxValue);  assert(result == "aleka")
    ...
    /**
      ...
      * -----------------------
      */
    test("`lettersAndNumbersOnly_?` test letters and numbers only") {
        var result = false
    ...
    test("`lettersAndNumbersOnly_?` test blank strings") {
        var result = false
    ...
    test("`removeAllButLettersAndNumbers`: rm all blank characters") {
        var result = ""
        result = removeAllButLettersAndNumbers("how to serve humans")
        assert(result == "howtoservehumans")

        result = removeAllButLettersAndNumbers("how to serve 11 humans")
        assert(result == "howtoserve11humans")

        result = removeAllButLettersAndNumbers("1how to 2 serve 3 humans 4")
        assert(result == "1howto2serve3humans4")

        result = removeAllButLettersAndNumbers("  how to ~s*er!ve~ humans  ")
        assert(result == "howtoservehumans")
    ...
    test("replaceAll: replace all characters other than letters and numbers") {
        // TODO need more test coverage here
        assert(replaceAll(
            "foo bar baz",
            "[^a-zA-Z0-9]",
            "*") == "foo*bar*baz")
    ...
    // TODO strings like "How " and " how " do not work properly, they remove the blank
    test("`capitalizeAllWordsInString` handle blanks in strings") (pending)
...
//    }
    ...
    test("getStackTraceAsString: handles basic throwable") {
        ...
        s = getStackTraceAsString(t)
        assert(s.startsWith("""java.lang.Throwable: foo
            |bar
            |baz""".stripMargin('|'))
        )
    }
    ...
    test("splitCamelCase works on FooBarBaz") {
        var s = splitCamelCase("FooBarBaz");  assert(s.equals("Foo Bar Baz"))
        s = splitCamelCase(" FooBarBaz");   assert(s.equals(" Foo Bar Baz"))
        s = splitCamelCase(" FooBarBaz ");  assert(s.equals(" Foo Bar Baz "))

        s = splitCamelCase(" FooBar ");  assert(s.equals(" Foo Bar "))
        s = splitCamelCase(" FooBar");  assert(s.equals(" Foo Bar"))
        s = splitCamelCase("FooBar");  assert(s.equals("Foo Bar"))

        s = splitCamelCase(" Foo ");  assert(s.equals(" Foo "))
        s = splitCamelCase(" Foo");  assert(s.equals(" Foo"))
        s = splitCamelCase("Foo");  assert(s.equals("Foo"))
    }
